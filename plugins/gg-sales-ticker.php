...
<?php
   ...
   */
   ...
   function gg_ticker_get_sales($request) {
          ...
          $table = $wpdb->prefix . 'gg_sales_ticker';
          $rows  = $wpdb->get_results($wpdb->prepare(
                     "SELECT item_title, sale_price, source, sold_at FROM {$table} ORDER BY sold_at DESC LIMIT %d", $limit
                 ), ARRAY_A);
   ...
   function gg_ticker_sync_from_webhook() {
          ...
          $rows = $wpdb->get_results(
                     "SELECT id, data FROM {$queue_table} WHERE status = 'done' AND queued_at > DATE_SUB(NOW(), INTERVAL 24 HOUR) ORDER BY queued_at DESC LIMIT 50",
                     ARRAY_A
                 );
          foreach ($rows as $row) {
   ...
   function gg_ticker_extract_title($data) {
          if (!empty($data['Item']['Title']) && is_string($data['Item']['Title'])) return sanitize_text_field($data['Item']['Title']);
          if (!empty($data['Transaction']['Item']['Title']) && is_string($data['Transaction']['Item']['Title'])) return sanitize_text_field($data['Transaction']['Item']['Title']);
          $item_id = $data['Item']['ItemID'] ?? ($data['Transaction']['Item']['ItemID'] ?? '');
          if ($item_id && function_exists('gg_find_product_by_ebay_id')) {
                     $product_id = gg_find_product_by_ebay_id((string)$item_id);
                     if ($product_id) { $post = get_post($product_id); if ($post) return $post->post_title; }
          }
          return '';
   }
   ...
   function gg_ticker_extract_price($data) {
          ...
          if ($item_id && function_exists('gg_find_product_by_ebay_id') && function_exists('wc_get_product')) {
                     $product_id = gg_find_product_by_ebay_id((string)$item_id);
                     if ($product_id) { $product = wc_get_product($product_id); if ($product) { $p = (float)$product->get_price(); if ($p > 0) return $p; } }
          }
          return 0;
   }
   ...
   function gg_ticker_cast_price($raw) {
          ...
          if (is_array($raw)) {
                     ...
                     foreach ($raw as $key => $val) { if ($key !== '@attributes' && is_numeric($val)) return (float)$val; }
          }
          return 0;
   }
   ...
   function gg_ticker_backfill_woo() {
          $orders  = wc_get_orders(['status' => ['processing', 'completed'], 'date_created' => '>' . date('Y-m-d', strtotime('-30 days')), 'limit' => 100, 'orderby' => 'date', 'order' => 'DESC']);
   ...
   function gg_ticker_debug() {
          ...
          return ['ticker_rows' => $ticker_rows, 'queue_done_count' => $queue_count, 'synced_count' => count(get_option('gg_ticker_synced_txns', [])), 'sync_lock_ttl' => get_transient('gg_ticker_sync_lock') ? 'locked' : 'free', 'version' => '2.1 - DB queue fix'];
